...
property_groups.map { | property_group | 
    ...
    property_group.map { | property | 
        ...
        Job.create!(
            title: [Faker::House.room.capitalize + ' ' + Faker::Lorem.words.to_sentence(words_connector: ' ' , last_word_connector: ' '), Faker::Lorem.words.to_sentence(words_connector: ' ' , last_word_connector: ' ') ].sample, 
            description: [nil, Faker::Address.secondary_address].sample,
            timeline: Faker::Address.city,
            is_accepted: Faker::Address.state,
            is_completed: Faker::Address.zip,
            property_id: property.id,
            contractor_profile_id: user.id # need to think through
        )
    }
}
